...
def get_forensic_score(y, sr, events, duration):
    """Hardened for ElevenLabs / Roger AI"""
    if len(events) < 3:
        return 0.90 # High suspicion for zero/single breath

    # 1. IBI Regularity (28%) - The 'Perfect Grid' detector
    ibis = np.diff(events)
    ibi_cv = np.std(ibis) / np.mean(ibis) if np.mean(ibis) > 0 else 0
    # If the spacing is too perfect (CV < 0.3), the AI score shoots up.
    p1 = np.clip(1.0 - (ibi_cv / 0.35), 0, 1)
    ...
    # 3. Breath Duration (12%) - Identical Inhale Lengths
    # AI often 'draws' the same breath shape. We check the variance of the 'silence' length.
    p3 = 1.0 if (len(set(np.round(ibis, 1))) < len(ibis) * 0.4) else 0.1

    # 4. Breath Presence (15%) - Excessive/Robotic breathing
    # Humans breathe roughly every 6-9 seconds. ElevenLabs 'Roger' often breathes every 2-3s.
    breaths_per_minute = (len(events) / duration) * 60
    p4 = 1.0 if breaths_per_minute > 25 or breaths_per_minute < 5 else 0.2

    # 5. Spectral Continuity (12%) - Digital Splicing
    # Look for sharp digital transients at the breath boundaries
    zcr = librosa.feature.zero_crossing_rate(y).flatten()
    p5 = np.clip(np.std(zcr) * 30, 0, 1)

    # 6. Breath Similarity (18%) - The Fingerprint
    mfccs = []
    for t in events:
    ...
    if len(mfccs) > 1:
        ...
        # Low distance = copy-pasted texture = AI
        p6 = np.clip(1.0 - (np.mean(dists) / 200), 0, 1)

    # Weighted Final Logic
    return (p1*0.28) + (p2*0.15) + (p3*0.12) + (p4*0.15) + (p5*0.12) + (p6*0.18)

st.title("🫁 PneumaForensic")

files = st.file_uploader("Upload Audio for Forensic Analysis", type=['wav', 'mp3', 'm4a'], accept_multiple_files=True)

if files:
    results = []
    for f in files:
        try:
            # Flatten to 1D to prevent blank-screen crashes
            ...
            for i, val in enumerate(rms):
                if val < threshold * 0.7:
                    t = times[i]
                    if t - last_t > 1.8: # Correct biological gap
    ...
    if results:
        # Table of scores
        st.table(pd.DataFrame([{"File": r['name'], "Forensic Confidence": f"{r['score']:.0%}"} for r in results]))

        # Forensic Timeline Visuals
        for r in results:
