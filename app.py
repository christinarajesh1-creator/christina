...
def forensic_analysis(y, sr, name):
    ...
    hop = 256
    # FIX: flatten() ensures the array is 1D for the convolution
    ...
    if 0.18 < ibi_cv < 0.25: ai_score += 30 
    if ibi_cv < 0.12: ai_score += 40        
    if sim_val < 1.1: ai_score += 35        
    if presence_ratio > 0.32: ai_score += 20 

    if is_biological:
        final_prob = min(35, ai_score) 
    else:
        final_prob = min(99, ai_score)
